...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def setup(self):
        ...
        fname_bitcoind = preferred_binary_path("universalmoleculed", "bitcoind")
        fname_bitcoincli = preferred_binary_path("universalmolecule-cli", "bitcoin-cli")
        fname_bitcoinutil = preferred_binary_path("universalmolecule-util", "bitcoin-util")
    ...
    def _initialize_chain(self):
        ...
        with open(cache_lock, "a+", encoding="utf8") as lockfile:
            ...
            if rebuild_cache:
                ...
                # Set mocktime just beyond UniversalMolecule regtest's 2*spacing window so
                # the first post-genesis block drops to powLimit difficulty. After
                # that, UniversalMolecule regtest keeps the easy target while mocktime
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in universalmolecule.conf
