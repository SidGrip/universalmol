 ...
 */
static const char* DEFAULT_PID_FILENAME = "universalmoleculed.pid";
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-auxpowmineraddress=<addr>", "UniversalMolecule AuxPoW payout address used by getauxblock without arguments. createauxblock accepts an explicit address and does not require this option.", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
...
bool AppInitParameterInteraction(const ArgsManager& args, bool use_syscall_sandbox)
{
    ...
    if (args.IsArgSet("-sandbox") && !args.IsArgNegated("-sandbox")) {
        ...
        LogPrintf("Experimental syscall sandbox enabled (-sandbox=%s): universalmoleculed will terminate if an unexpected (not allowlisted) syscall is invoked.\n", sandbox_arg);
    }
#endif // USE_SYSCALL_SANDBOX
