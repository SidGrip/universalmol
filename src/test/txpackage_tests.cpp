...
static constexpr CAmount PACKAGE_FEE{1'000};
static constexpr CAmount PACKAGE_HIGH_FEE{10'000};
static constexpr CAmount PACKAGE_LOW_FEE{200};

static CAmount OutputMinusFee(const CTransactionRef& tx, CAmount fee, uint32_t output_index = 0)
{
    const CAmount input_value = tx->vout.at(output_index).nValue;
    assert(input_value > fee);
    return input_value - fee;
}
...
BOOST_FIXTURE_TEST_CASE(package_validation_tests, TestChain100Setup)
{
    ...
    auto mtx_parent = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    ...
                                                    /*output_amount=*/OutputMinusFee(m_coinbase_txns[0], PACKAGE_FEE), /*submit=*/false);
    ...
    auto mtx_child = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent, /*input_vout=*/0,
                                                   ...
                                                   /*output_amount=*/OutputMinusFee(tx_parent, PACKAGE_FEE), /*submit=*/false);
    ...
    BOOST_CHECK(it_parent->second.m_effective_feerate.value().GetFee(GetVirtualTransactionSize(*tx_parent)) == PACKAGE_FEE);
    ...
    BOOST_CHECK(it_child->second.m_effective_feerate.value().GetFee(GetVirtualTransactionSize(*tx_child)) == PACKAGE_FEE);
...
BOOST_FIXTURE_TEST_CASE(noncontextual_package_tests, TestChain100Setup)
{
    ...
    // Parent and Child Package
    {
        auto mtx_parent = CreateValidMempoolTransaction(m_coinbase_txns[0], 0, 0, coinbaseKey, spk,
                                                        OutputMinusFee(m_coinbase_txns[0], PACKAGE_FEE), /*submit=*/false);
        CTransactionRef tx_parent = MakeTransactionRef(mtx_parent);

        auto mtx_child = CreateValidMempoolTransaction(tx_parent, 0, 101, placeholder_key, spk2,
                                                       OutputMinusFee(tx_parent, PACKAGE_FEE), /*submit=*/false);
        CTransactionRef tx_child = MakeTransactionRef(mtx_child);
    ...
    // 24 Parents and 1 Child
    {
        ...
        for (int i{0}; i < 24; ++i) {
            auto parent = MakeTransactionRef(CreateValidMempoolTransaction(m_coinbase_txns[i + 1],
                                             0, 0, coinbaseKey, spk, OutputMinusFee(m_coinbase_txns[i + 1], PACKAGE_FEE), false));
            package.emplace_back(parent);
            child.vin.push_back(CTxIn(COutPoint(parent->GetHash(), 0)));
        }
        child.vout.push_back(CTxOut((m_coinbase_txns[1]->vout[0].nValue - PACKAGE_FEE) * 24 - PACKAGE_FEE, spk2));
    ...
    // 2 Parents and 1 Child where one parent depends on the other.
    {
        ...
        const CAmount split_value = m_coinbase_txns[0]->vout[0].nValue / 4;
        mtx_parent.vout.push_back(CTxOut(split_value, spk));
        mtx_parent.vout.push_back(CTxOut(split_value, spk2));
        CTransactionRef tx_parent = MakeTransactionRef(mtx_parent);

        CMutableTransaction mtx_parent_also_child;
        mtx_parent_also_child.vin.push_back(CTxIn(COutPoint(tx_parent->GetHash(), 0)));
        mtx_parent_also_child.vout.push_back(CTxOut(split_value - PACKAGE_FEE, spk));
        ...
        mtx_child.vout.push_back(CTxOut((2 * split_value) - (2 * PACKAGE_FEE), spk));
        CTransactionRef tx_child = MakeTransactionRef(mtx_child);
...
BOOST_FIXTURE_TEST_CASE(package_submission_tests, TestChain100Setup)
{
    ...
    for (size_t i{0}; i < 10; ++i) {
        auto mtx = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[i + 25], /*input_vout=*/0,
                                                 /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                 /*output_destination=*/parent_locking_script,
                                                 /*output_amount=*/OutputMinusFee(m_coinbase_txns[i + 25], PACKAGE_FEE), /*submit=*/false);
    ...
    auto mtx_grandchild = CreateValidMempoolTransaction(/*input_transaction=*/tx_child, /*input_vout=*/0,
                                                       ...
                                                       /*output_amount=*/OutputMinusFee(tx_child, PACKAGE_FEE), /*submit=*/false);
    ...
    // Submit package with parent + child.
    {
        ...
        BOOST_CHECK(it_parent->second.m_effective_feerate == CFeeRate(PACKAGE_FEE, GetVirtualTransactionSize(*tx_parent)));
        BOOST_CHECK_EQUAL(it_parent->second.m_wtxids_fee_calculations.value().size(), 1);
        BOOST_CHECK_EQUAL(it_parent->second.m_wtxids_fee_calculations.value().front(), tx_parent->GetWitnessHash());
        BOOST_CHECK(it_child != submit_parent_child.m_tx_results.end());
        BOOST_CHECK(it_child->second.m_state.IsValid());
        BOOST_CHECK(it_child->second.m_effective_feerate == CFeeRate(PACKAGE_FEE, GetVirtualTransactionSize(*tx_child)));
        BOOST_CHECK_EQUAL(it_child->second.m_wtxids_fee_calculations.value().size(), 1);
        BOOST_CHECK_EQUAL(it_child->second.m_wtxids_fee_calculations.value().front(), tx_child->GetWitnessHash());
...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    mtx_child1.vout[0].nValue = OutputMinusFee(ptx_parent, PACKAGE_FEE);
    ...
    auto mtx_grandchild = CreateValidMempoolTransaction(/*input_transaction=*/ptx_child2, /*input_vout=*/0,
                                                        ...
                                                        /*output_destination=*/grandchild_locking_script,
                                                        /*output_amount=*/OutputMinusFee(ptx_child2, PACKAGE_FEE), /*submit=*/false);
    ...
    auto mtx_parent1 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                     ...
                                                     /*output_amount=*/OutputMinusFee(m_coinbase_txns[1], PACKAGE_FEE), /*submit=*/true);
    ...
    auto mtx_grandparent2 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[2], /*input_vout=*/0,
                                                          ...
                                                          /*output_amount=*/OutputMinusFee(m_coinbase_txns[2], PACKAGE_FEE), /*submit=*/true);
    ...
    mtx_parent2_v1.vout[0].nValue = OutputMinusFee(ptx_grandparent2, PACKAGE_FEE);
    ...
    auto mtx_parent3 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[3], /*input_vout=*/0,
                                                     /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                     /*output_destination=*/acs_spk,
                                                     /*output_amount=*/m_coinbase_txns[3]->vout[0].nValue, /*submit=*/false);
    ...
    mtx_mixed_child.vout.push_back(CTxOut(ptx_parent1->vout[0].nValue + ptx_parent2_v1->vout[0].nValue + ptx_parent3->vout[0].nValue - PACKAGE_FEE, mixed_child_spk));
    ...
    // child should be accepted
    {
        ...
        const CFeeRate expected_feerate(PACKAGE_FEE, GetVirtualTransactionSize(*ptx_parent3) + GetVirtualTransactionSize(*ptx_mixed_child));
...
BOOST_FIXTURE_TEST_CASE(package_cpfp_tests, TestChain100Setup)
{
    ...
    const CAmount child_value{parent_value - PACKAGE_HIGH_FEE};
    ...
    // fee deltas. This should be taken into account. De-prioritise the parent transaction by the child fee,
    // bringing the package feerate to 0.
    m_node.mempool->PrioritiseTransaction(tx_parent->GetHash(), -PACKAGE_HIGH_FEE);
    {
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
    ...
    // Package CPFP: Even though the parent pays 0 absolute fees, the child pays enough
    // enough for the package feerate to meet the threshold.
    {
        ...
        BOOST_CHECK(it_child->second.m_result_type == MempoolAcceptResult::ResultType::VALID);
        BOOST_CHECK(it_child->second.m_base_fees.value() == PACKAGE_HIGH_FEE);
    ...
    // This package just pays a tiny fee. This would be enough to pay for the child alone,
    ...
    auto mtx_child_cheap = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent_cheap, /*input_vout=*/0,
                                                         /*input_height=*/101, /*input_signing_key=*/child_key,
                                                         /*output_destination=*/child_spk,
                                                         /*output_amount=*/coinbase_value - PACKAGE_LOW_FEE, /*submit=*/false);
    ...
    // Cheap package should fail with package-fee-too-low.
    {
        ...
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
        const CFeeRate child_feerate(PACKAGE_LOW_FEE, GetVirtualTransactionSize(*tx_child_cheap));
        BOOST_CHECK(child_feerate.GetFeePerK() > 1000);
        const CFeeRate expected_feerate(PACKAGE_LOW_FEE,
    ...
    m_node.mempool->PrioritiseTransaction(tx_child_cheap->GetHash(), PACKAGE_HIGH_FEE);
    // Now that the child's fees have increased enough, the cheap package should succeed.
    {
        ...
        const CFeeRate expected_feerate(PACKAGE_HIGH_FEE + PACKAGE_LOW_FEE,
            GetVirtualTransactionSize(*tx_parent_cheap) + GetVirtualTransactionSize(*tx_child_cheap));
        ...
        BOOST_CHECK(it_child->second.m_result_type == MempoolAcceptResult::ResultType::VALID);
        BOOST_CHECK(it_child->second.m_base_fees.value() == PACKAGE_LOW_FEE);
        BOOST_CHECK(it_child->second.m_effective_feerate.value() == expected_feerate);
    ...
    const CAmount high_parent_fee{PACKAGE_HIGH_FEE};
    ...
    // Parent pays enough and child pays none. The parent should be accepted without the child.
    {
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
