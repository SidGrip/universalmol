...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
...

    {
        ...
        // UniversalMolecule uses its own subsidy schedule, so its halving
        // interval is intentionally unreachable in regtest.
    ...
    // UniversalMolecule 0.25.2 enforces relative locks during block assembly here, so
    ...
    // The extra height keeps the absolute and relative lock checks aligned under UniversalMolecule maturity.
    for (int i = 0; i < CBlockIndex::nMedianTimeSpan; ++i) {
        CBlockIndex* ancestor{Assert(m_node.chainman->ActiveChain().Tip()->GetAncestor(m_node.chainman->ActiveChain().Tip()->nHeight - i))};
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    const int legacy_launch_height = 1440;
    const int blocks_to_import = legacy_launch_height + COINBASE_MATURITY + 10;
    int baseheight = 0;
    ...
    for (int block_num = 0; block_num < blocks_to_import; ++block_num) {
        ...
        CBlock *pblock = &pblocktemplate->block; // pointer for convenience
        {
            ...
            // UniversalMolecule regtest starts from the historical genesis nBits. Push
            ...
            if (m_node.chainman->ActiveChain().Height() >= legacy_launch_height && txFirst.size() < 4)
            {
